export function User1WebRTC(iceConfig, log, sendToServer, hangUpCall) {

  var pc = null;
  var user1Video = document.getElementById("user1Video");
  ...
  this.handleVideoOffer = async (msg) => {
    ...
    if (!user1Video.srcObject) {
      user1Video.srcObject = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
    }

    user1Video.srcObject.getTracks().forEach(track => pc.addTrack(track, user1Video.srcObject));
